...
CREATE INDEX IF NOT EXISTS idx_app_analyses_user_id ON app_analyses(user_id);
CREATE INDEX IF NOT EXISTS idx_app_analyses_status ON app_analyses(status);
...
CREATE TABLE IF NOT EXISTS next_best_steps (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS watch_mode_subscriptions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  analysis_id UUID NOT NULL REFERENCES app_analyses(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_watch_subscriptions_user_id ON watch_mode_subscriptions(user_id);
CREATE INDEX IF NOT EXISTS idx_watch_subscriptions_analysis_id ON watch_mode_subscriptions(analysis_id);
CREATE INDEX IF NOT EXISTS idx_watch_subscriptions_status ON watch_mode_subscriptions(status);
...
CREATE INDEX IF NOT EXISTS idx_prepared_changes_analysis_id ON prepared_changes(analysis_id);
...
COMMENT ON TABLE users IS 'User accounts for DevOpsify.ai';
COMMENT ON TABLE app_analyses IS 'Analysis records for user applications';
COMMENT ON TABLE risk_scenarios IS 'Reference data for risk detection';
COMMENT ON TABLE platform_recommendations IS 'Reference data for platform suggestions';
COMMENT ON TABLE alerts IS 'User alerts and notifications';
COMMENT ON TABLE watch_mode_subscriptions IS 'Watch mode monitoring subscriptions';
