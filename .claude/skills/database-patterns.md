...
const result = await query("DELETE FROM sessions WHERE id = $1 RETURNING *", [
  sessionToken,
]);
...
// Update JSONB field
await query("UPDATE app_analyses SET stack_detection = $1 WHERE id = $2", [
...
const allAlerts = await getMany("SELECT * FROM alerts WHERE user_id = $1", [
  userId,
]);
...
const user = await getOne("SELECT id, email, name FROM users WHERE id = $1", [
  userId,
]);
```
